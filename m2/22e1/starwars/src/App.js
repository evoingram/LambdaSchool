        ...
        if (previousLink) {
          axios
            ...
            .then(
              response => {
                setCharacters(response.data.results);
                if (response.data.previous) { setPreviousLink(response.data.previous) };
  ...
  const PreviousButton = () => {   
  {
      if (previousLink != null) {
        pArray.map(function (item) {
          return (
            <PaginationBtn><div class="button"><a href={previousLink}>Previous Page</a></div></PaginationBtn>
          )
        })
      }
      else { return null };
  ...
  const NextButton = () => {
    if (nextLink != null) {

      {
        pArray.map(function (item) {
          return (
      <PaginationBtn><div class="button"><a href={nextLink}>Next Page</a></div></PaginationBtn>
          )
        })
      }
    }
  ...
  return (
    ...
    <SWH1>React Wars</SWH1>
      ...
      {characters.map(function (item) {
        ...
        return <div key = {item}>          
      <div>
        <p><BioData>Name:  </BioData><CustomData>{item.name}</CustomData></p>
        <p><BioData>Birth Year:  </BioData><CustomData>{item.birth_year}</CustomData></p>
        <p><BioData>Gender:  </BioData><CustomData>{item.gender}</CustomData></p>
        <p>
          <SWLinks href={item.homeworld} target="_blank">Homeworld URL</SWLinks>
        </p>
        <p>
          <SWLinks href={item.films} target="_blank">Films URL</SWLinks>
        </p>
        <p>
          <SWLinks href={item.starships} target="_blank">Starships URL</SWLinks>
        </p>
      </div>
      </div> 
        })}
